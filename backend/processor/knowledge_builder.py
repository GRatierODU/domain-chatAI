...
class KnowledgeBuilder:
    """
    Builds knowledge base from processed content
    """
    ...
    def _create_knowledge_chunks(self, page: CrawledPage, content: ProcessedContent) -> List[Dict]:
        """
        ...
        for text_chunk in content.text_chunks:
            ...
            if 'embedding_text' in text_chunk:
                chunk['text'] = text_chunk['embedding_text']
    ...
    def _create_page_overview(self, page: CrawledPage, content: ProcessedContent) -> Dict:
        """
        ...
        if understanding.get('main_sections'):
            overview_parts.append(f"Main sections: {', '.join(understanding['main_sections'])}")
            
        if understanding.get('key_information'):
            overview_parts.append(f"Key information: {understanding['key_information']}")
    ...
    def _structured_data_to_text(self, data: Dict) -> str:
        ...
        if data.get('type') == 'table':
            ...
            if isinstance(data.get('content'), dict):
                for key in ['name', 'description', 'address', 'telephone', 'email', 'priceRange']:
    ...
    def _interaction_to_text(self, interaction: Dict) -> str:
        ...
        if interaction.get('type') == 'form':
            ...
            text_parts.append(f"Call to action: {interaction.get('text', 'Click here')}")
            if interaction.get('action'):
                text_parts.append(f"Action: {interaction['action']}")
                
        return " ".join(text_parts)
        
    def _save_collection_metadata(self, collection_name: str, domain: str, pages: List[CrawledPage]):
        """
        ...
        # Save to a metadata collection
        try:
            metadata_collection = self.chroma_client.get_or_create_collection('_metadata')
